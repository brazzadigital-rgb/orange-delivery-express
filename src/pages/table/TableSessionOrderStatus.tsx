...
export default function TableSessionOrderStatus() {
  const { sessionToken, orderId } = useParams<{ sessionToken: string; orderId: string }>();
  ...
  const checkBeverageOnly = async (orderItems: OrderItemData[]) => {
    ...
    const { data: categories } = await supabase.from('categories').select('id, slug, name').in('id', categoryIds as string[]);
    if (!categories) return;
    const allBeverage = categories.every(c =>
      c.slug?.toLowerCase().includes('bebida') || c.slug?.toLowerCase().includes('drink') ||
      c.name?.toLowerCase().includes('bebida') || c.name?.toLowerCase().includes('drink')
    );
    setIsBeverageOnly(allBeverage);
  };

  useEffect(() => { fetchOrder(); fetchItems(); }, [orderId]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (!isActive) return;
      await fetchOrder();
      if (isActive) { pollInterval = Math.min(pollInterval * 1.3, 15000); timeoutId = setTimeout(poll, pollInterval); }
    };
    const channel = supabase.channel(`table-order-${orderId}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${orderId}` }, () => { fetchOrder(); pollInterval = 3000; })
      .subscribe();
  ...
  if (!order) return <div className="min-h-screen flex items-center justify-center p-6 text-center"><p className="text-muted-foreground">Pedido não encontrado</p></div>;
      ...
      {showReadyAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowReadyAlert(false)}>
          <div className="bg-background rounded-2xl p-8 text-center max-w-sm mx-4 animate-in zoom-in-95">
            ...
            <h2 className="text-2xl font-bold mb-2">{isBeverageOnly ? 'Suas bebidas chegaram!' : 'Seu pedido está pronto!'}</h2>
            <p className="text-muted-foreground mb-6">{isBeverageOnly ? 'Suas bebidas foram servidas na mesa.' : 'Aguarde o garçom servir na sua mesa.'}</p>
      ...
      <div className="min-h-screen bg-background pb-24">
        <div className="sticky top-0 z-40 px-4 py-3 flex items-center gap-3 bg-background/95 backdrop-blur-sm border-b border-border/50">
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">{format(new Date(order.created_at), "dd/MM 'às' HH:mm", { locale: ptBR })}</p>
          </div>
        ...
        <div className="px-4 space-y-6 mt-4">
          {isRejectedOrCanceled && (
            <div className="flex items-center gap-3 px-4 py-4 rounded-xl bg-destructive/10 border border-destructive/30">
              ...
              <div>
                <p className="font-bold text-destructive">{order.status === 'rejected' ? 'Pedido Recusado' : 'Pedido Cancelado'}</p>
                <p className="text-sm text-muted-foreground mt-0.5">
                  {order.status === 'rejected' ? 'Infelizmente o restaurante não pôde aceitar seu pedido.' : 'Este pedido foi cancelado.'}
                </p>
                {(order as any).reject_reason && <p className="text-sm mt-1">Motivo: {(order as any).reject_reason}</p>}
              </div>
            </div>
          )}
            ...
            <section className="card-premium p-4">
              ...
              <div className="space-y-0">
                {statusOrder.map((status, index) => {
                  ...
                  return (
                    <div key={status} className={cn("flex gap-4 transition-all duration-500", isCurrent && "scale-[1.02]")}>
                      <div className="flex flex-col items-center">
                        <div className="relative">
                          ...
                          <div className={cn('relative w-8 h-8 rounded-full transition-all duration-300 flex items-center justify-center',
                            isPast && 'bg-primary', isCurrent && 'bg-primary scale-110 shadow-lg shadow-primary/40',
                            !isPast && !isCurrent && 'bg-muted border-2 border-border')}>
                            <StatusIcon className={cn("w-4 h-4 transition-all duration-300",
                        ...
                        {index < statusOrder.length - 1 && (
                          <div className="relative w-0.5 h-12 mt-1">
                            ...
                            <div className={cn('absolute top-0 left-0 w-full bg-primary transition-all duration-500 ease-out',
                      ...
                      <div className="flex-1 -mt-0.5 pb-6">
                        <p className={cn('font-medium transition-all duration-300',
                          isPast && 'text-foreground', isCurrent && 'text-primary font-semibold',
