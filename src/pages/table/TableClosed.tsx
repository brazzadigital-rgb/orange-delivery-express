      ...
      <div className="text-center max-w-sm mx-auto space-y-8 relative z-10">
        ...
        >
          <div className="relative inline-block">
            ...
            <div
              className={`absolute -top-1 -right-1 transition-all duration-500 delay-500 ${
                show ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-0 rotate-45'
              }`}
            >
        ...
        {/* Title */}
        <div
          className={`space-y-3 transition-all duration-700 ease-out delay-300 ${
        ...
        <div className="flex items-center justify-center gap-2">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`w-7 h-7 text-primary fill-primary transition-all duration-500 ease-out ${
                showStars ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
              }`}
              style={{
                transitionDelay: `${i * 120}ms`,
                filter: showStars ? 'drop-shadow(0 0 6px hsl(var(--primary) / 0.4))' : 'none',
              }}
            />
          ))}
        </div>
        ...
        {/* Footer */}
        <div
          className={`flex items-center justify-center gap-1.5 text-muted-foreground/50 text-xs pt-2 transition-all duration-700 ${
