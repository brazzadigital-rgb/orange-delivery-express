...
CREATE TABLE public.orders (
  ...
  cancel_reason TEXT,
  reject_reason TEXT,
  cash_change_needed BOOLEAN DEFAULT false,
  cash_change_for NUMERIC,
  cash_change_amount NUMERIC,
  channel TEXT NOT NULL DEFAULT 'internal',
  external_order_id TEXT,
  merchant_id_ifood TEXT,
  raw_payload JSONB,
  loyalty_points_earned INTEGER NOT NULL DEFAULT 0,
  loyalty_points_spent INTEGER NOT NULL DEFAULT 0,
  loyalty_reward_applied JSONB,
  loyalty_earn_processed BOOLEAN NOT NULL DEFAULT false,
  loyalty_spend_processed BOOLEAN NOT NULL DEFAULT false,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE INDEX idx_order_events_order_id_created_at ON public.order_events(order_id, created_at);
...
CREATE POLICY "Users can view order items for their orders" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = order_items.order_id AND orders.user_id = auth.uid()));
CREATE POLICY "Users can create order items for their orders" ON public.order_items FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = order_items.order_id AND orders.user_id = auth.uid()));
...
CREATE POLICY "Drivers can view assigned order items" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = order_items.order_id AND orders.driver_id = auth.uid()));
CREATE POLICY "Users can view events for their orders" ON public.order_events FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = order_events.order_id AND orders.user_id = auth.uid()));
...
CREATE POLICY "Drivers can view events for assigned orders" ON public.order_events FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = order_events.order_id AND orders.driver_id = auth.uid()));
CREATE POLICY "Drivers can create events for assigned orders" ON public.order_events FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = order_events.order_id AND orders.driver_id = auth.uid()));
CREATE POLICY "Drivers can insert their location" ON public.driver_locations FOR INSERT WITH CHECK (auth.uid() = driver_id);
CREATE POLICY "Drivers can update their location" ON public.driver_locations FOR UPDATE USING (auth.uid() = driver_id);
CREATE POLICY "Customers can view driver location for their active delivery" ON public.driver_locations FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE orders.id = driver_locations.order_id AND orders.user_id = auth.uid() AND orders.status = 'out_for_delivery'));
CREATE POLICY "Admins can view all driver locations" ON public.driver_locations FOR SELECT USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'staff'));
...
CREATE POLICY "Admins can create notifications" ON public.notifications FOR INSERT WITH CHECK (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'staff'));
...
-- FK from user_roles to profiles
...
-- FK from orders to profiles
ALTER TABLE public.orders ADD CONSTRAINT orders_user_id_profiles_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE RESTRICT;
